...
export function SymptomChips({ symptoms, onToggleSymptom, onRemove, onOpenPicker }: SymptomChipsProps) {
  ...
  const common = KNOWN_SYMPTOMS.filter(symptom => COMMON_IDS.includes(symptom.id) && !selected.has(symptom.id));
  return <div className="space-y-3">
    {symptoms.length > 0 && <div className="space-y-2"><p className="text-xs font-semibold text-[var(--text-secondary)]">En tu registro · toca para quitar</p><div className="flex flex-wrap gap-2">{symptoms.map(symptom => <button type="button" key={symptom.id} aria-label={`Eliminar ${symptom.name}`} onClick={() => onRemove(symptom.id)} className={`flex items-center gap-2 ${modalChoice} ${modalSelected}`}><Check size={16} aria-hidden="true"/><span>{symptom.name}</span><X size={16} className="shrink-0" aria-hidden="true" /></button>)}</div></div>}
    <p className="text-xs text-[var(--text-secondary)]">Toca una sensación para añadirla. Puedes cambiarla cuando quieras.</p>
