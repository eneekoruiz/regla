...
export function PwaInstallModal({ onClose }: { onClose: () => void }) {
  ...
  return <ModalFrame isOpen onClose={onClose} title="Aura, más cerca de ti" description="Un acceso directo a tu diario desde la pantalla de inicio." closeDisabled={pending} errorMessage={error}
    footer={<button type="button" className="aura-button primary" disabled={pending} onClick={canPrompt && !installed ? async () => { onClose(); await install(); } : onClose}>{canPrompt && !installed ? <><Download size={18}/>{pending ? 'Abriendo instalación…' : 'Instalar Aura'}</> : 'Entendido'}</button>}>
