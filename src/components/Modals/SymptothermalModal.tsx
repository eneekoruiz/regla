...
export function SymptothermalModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const save = () => {
    ...
    try {
      logSymptothermalForDate(selectedDate, { cervicalMucus: mucus, bbt: value });
  ...
  return <ModalFrame isOpen={isOpen} onClose={onClose} title="Registro sintotérmico" description={new Date(selectedDate + 'T12:00:00').toLocaleDateString('es-ES', { dateStyle: 'long' })} errorMessage={error} onClearError={() => setError('')}
    ...
    <label className="block space-y-2 text-sm font-semibold">Temperatura basal (°C)<input type="text" inputMode="decimal" autoComplete="off" autoCorrect="off" spellCheck={false} value={temperature} onChange={event => { setTemperature(event.target.value); setError(''); }} placeholder="Sin registrar" aria-invalid={Boolean(error)} aria-describedby={error ? 'temperature-error' : undefined} className={`${modalField} ${error ? 'field-shake' : ''}`} /></label>
