...
import { Download, Upload, LogOut, Trash2, Check, Bell, ChevronRight, UserRound, Shield, Moon } from 'lucide-react';
  ...
  return <>
    <ModalFrame isOpen onClose={close} title="Ajustes"
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4" aria-label="Secciones de ajustes">
        {([['account', 'Cuenta'], ['cycle', 'Ciclo'], ['privacy', 'Privacidad y datos'], ['notifications', 'Avisos']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={tab === value} onClick={() => { setTab(value); setError(''); setMessage(''); }} className={'aura-button' + (tab === value ? ' primary' : '')}>{label}</button>)}
      </div>

      {tab === 'account' && <div className="space-y-4">
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-root)] p-4 space-y-2">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Cuenta conectada</p>
              <p className="text-base font-semibold text-[var(--text-primary)]">{user?.email || settings.userName || 'Modo privado local'}</p>
            </div>
          ...
          <p className="text-xs text-[var(--text-secondary)]">{Object.keys(logs).length} días registrados en este dispositivo.</p>
        </div>
        <button type="button" onClick={() => setTool('install')} className={modalSecondaryButton}><Download size={17} aria-hidden="true"/>Instalar Aura en este dispositivo</button>
      ...
      {tab === 'cycle' && <div className="space-y-4">
        <p className="text-xs text-[var(--text-secondary)]">Configura los parámetros para las estimaciones de tu ciclo y accede a tu perfil de hábitos.</p>
      ...
      {tab === 'privacy' && <div className="space-y-4">
        ...
        <div className="border-t border-[var(--border-subtle)] pt-4 space-y-2">
          <p className="text-sm font-semibold text-[var(--text-primary)]">Copias de seguridad y datos</p>
          <p className="text-xs text-[var(--text-secondary)]">Guarda o traslada tu historial de forma segura.</p>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={download} className={modalSecondaryButton}><Download size={17} aria-hidden="true" />Exportar copia</button>
            <button type="button" onClick={() => setEncryptedBackupOpen(true)} className={modalSecondaryButton}><Download size={17} aria-hidden="true" />Copia cifrada</button>
            <button type="button" onClick={() => setTool('import')} className={modalSecondaryButton}><Upload size={17} aria-hidden="true" />Importar o restaurar</button>
          </div>
        </div>

        <div className="flex flex-col gap-2 border-t border-[var(--border-subtle)] pt-4">
          <button type="button" onClick={() => setTool('report')} className={modalSecondaryButton}><Download size={17} aria-hidden="true" />Informe de salud para consulta</button>
          <button type="button" onClick={() => setTool('legal')} className={modalSecondaryButton}><Shield size={17} aria-hidden="true" />Privacidad y límites de Aura</button>
